...
var ExhaustiveDeps = {
  ...
  create: function (context) {
    ...
    var ignoreThisDependency = context.options && context.options[0] && context.options[0].ignoreThisDependency || 'never';
    var options = {
      additionalHooks: additionalHooks,
      enableDangerousAutofixThisMayCauseInfiniteLoops: enableDangerousAutofixThisMayCauseInfiniteLoops,
      ignoreThisDependency: ignoreThisDependency
    };
      ...
      function gatherDependenciesRecursively(currentScope) {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            var dependencyNode = getDependency(referenceNode, ignoreThisDependency);
...
function getDependency(node, ignoreThisDependency) {
  if ((node.parent.type === 'MemberExpression' || node.parent.type === 'OptionalMemberExpression') && node.parent.object === node && node.parent.property.name !== 'current' && !node.parent.computed && !(ignoreThisDependency !== 'always' && !(ignoreThisDependency === 'props' && node.name === 'props') && node.parent.parent != null && (node.parent.parent.type === 'CallExpression' || node.parent.parent.type === 'OptionalCallExpression') && node.parent.parent.callee === node.parent)) {
